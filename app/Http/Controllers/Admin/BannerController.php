...
class BannerController extends Controller
{
    ...
    {
        $request->validate(
            ['banner_name'=>'required','banner_link'=>'required','banner_image'=>'required|image','banner_odernum'=>'required'],
    ...
    function post_edit_banner(Request $request,$id){
        ...
        if($file_image)
            {
                $file_name=$file_image->getClientOriginalName();
                $file_image->move('public/backend/images/uploads/banners',$file_name);
                $banner->image=$file_name;
            }
        $banner->save();

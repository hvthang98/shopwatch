...
class BrandController extends Controller
{
    public function product_of_brand(Request $request, $id){
    	...
    	//  $pro=[];
    	// $product=Products::where('brands_id',$id);
    	 ...
    	 if($request->has('pri')){
    	 	...
    	 	switch ($tempt) {
    	 		case 1:
    	 		$product=$product->paginate(2)->appends('sort',request('sort'));
    	 		break;
    	 		case 2:
    	 		$product=$product->paginate(2)->appends('pri',request('pri'));
    	 		break;
    	 		case 3:
    	 		$product=$product->paginate(2)->appends('gender',request('gender'));
    	 		break;
    	 		case 4:
    	 		$product=$product->paginate(2)->appends('sort',request('sort'))->appends('pri',request('pri'));
    	 		break;
    	 		case 5:
    	 		$product=$product->paginate(2)->appends('gender',request('gender'))->appends('sort',request('sort'));
    	 		break;
    	 		case 6:
    	 		$product=$product->paginate(2)->appends('gender',request('gender'))->appends('pri',request('pri'));
    	 		break;
    	 		case 7:
    	 		$product=$product->paginate(2)->appends('gender',request('gender'))->appends('pri',request('pri'))->appends('sort',request('sort'));
    	 		break;
    	 		default:
...
//  $img=[];//hình ảnh sản phẩm
    	...
    	//  	$pr = Products::find($pro->id)->imgProduct->where('level',1);
    	//  	foreach ($pr as  $p) {
    	//  		$img[]=$p->image;
    	//  	}
